...
Be concise, clear, and highly precise. Use formatted markdown, lists, and tables when helpful.`;
        ...
        // Parse reminders if present in responseText
        let cleanedResponseText = responseText;
        const reminderRegex = /\[\[REMINDER:\s*text="([^"]+)"\s*delay="([^"]+)"\]\]/i;
        const reminderMatch = responseText.match(reminderRegex);
        if (reminderMatch) {
          const tag = reminderMatch[0];
          // Schedule reminder
          scheduleReminderFromTag(tag);
          // Strip out the tag from the text
          cleanedResponseText = responseText.replace(reminderRegex, '').trim();
