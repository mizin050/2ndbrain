...
- IMPORTANT: Set reminders ONLY when the user explicitly requests you to remember or remind them of something, or when it's clearly to remember something important they mentioned. Do NOT generate reminders for general conversational statements.
- If the user requests a reminder for a special recurring or one-time calendar date (such as birthdays, anniversaries, weddings, or major milestones):
  You MUST generate MULTIPLE reminder tags to alert them in advance. Calculate the absolute dates based on the Current Local Time above and generate precisely these 4 tags:
  1. One month before the event: [[REMINDER: text="[Event Name] in 1 month" delay="[Calculated absolute date 1 month before, formatted as YYYY-MM-DD 09:00]"]]
  2. One week before the event: [[REMINDER: text="[Event Name] in 1 week" delay="[Calculated absolute date 1 week before, formatted as YYYY-MM-DD 09:00]"]]
  3. One day before the event: [[REMINDER: text="[Event Name] tomorrow" delay="[Calculated axis date 1 day before, formatted as YYYY-MM-DD 09:00]"]]
  4. On the day of the event: [[REMINDER: text="[Event Name] today!" delay="[Calculated absolute date of event, formatted as YYYY-MM-DD 09:00]"]]
  
- For general single reminders (e.g. "remind me to check the oven in 10 minutes"), append a single tag at the end of your response:
  [[REMINDER: text="Your reminder content" delay="duration_or_time_string"]]
  
  Examples:
  - "remind me in 10 seconds to drink water" -> [[REMINDER: text="Drink water" delay="10"]]
  - "remind me in 5 minutes to buy milk" -> [[REMINDER: text="Buy milk" delay="300"]]
  - "remind me to check oven at 3:00 PM" -> [[REMINDER: text="Check oven" delay="15:00"]]
...
If the query cannot be answered using the retrieved context or conversation history, use your general knowledge but mention that it is not in the local Second Brain notes.`;
        ...
        const reminderTagRegex = /\[\[REMINDER:\s*([\s\S]+?)\s*\]\]/gi;
        const matches = [...responseText.matchAll(reminderTagRegex)];
  ...
  */
  ...
  window.syncWidgetReminders = function() {
  ...
  window.scheduleReminderFromTag = function(tagText) {
    ...
    if (/^\d+$/.test(delayVal)) {
      ...
      // 1. Parse flexible duration formats with support for s, m, h, d, w, mo, y
      const durationMatch = delayVal.match(/^(\d+(?:\.\d+)?)\s*(s|sec|second|seconds|m|min|minute|minutes|h|hr|hour|hours|d|day|days|w|wk|week|weeks|mo|mon|month|months|y|yr|year|years)$/i);
      if (durationMatch) {
        ...
        if (unit.startsWith('s')) {
          ...
          targetTime += value * 24 * 60 * 60 * 1000;
        } else if (unit.startsWith('w')) {
          targetTime += value * 7 * 24 * 60 * 60 * 1000;
        } else if (unit.startsWith('mo')) {
          targetTime += value * 30 * 24 * 60 * 60 * 1000;
        } else if (unit.startsWith('y')) {
          targetTime += value * 365 * 24 * 60 * 60 * 1000;
        }
      } else {
        // 2. Try parsing as an absolute date first if it contains a slash, dash, year, or month names
        const looksLikeAbsoluteDate = /[\/\-]|(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/i.test(delayVal);
        const parsedDate = Date.parse(delayVal);
        if (looksLikeAbsoluteDate && !isNaN(parsedDate)) {
          targetTime = parsedDate;
        } else {
          // 3. Fall back to wall-clock time format (e.g., "3:00 PM", "15:00")
    ...
    // Dispatch reminder to Capacitor Native LocalNotifications or Service Worker fallback
    const scheduleBackgroundNotification = async (title, body, targetTime) => {
      if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.LocalNotifications) {
        try {
          ...
          console.warn("Capacitor LocalNotification failed, falling back to Service Worker:", e);
        }
      }
    ...
    if (typeof syncWidgetReminders === 'function') {
      syncWidgetReminders();
    }
  };
